...
# from .celery_tasks import ingest_file, update_files
# from core.providers.orchestration.hatchet import CeleryOrchestrationProvider
from core.main.hatchet.ingestion_workflow import ingest_file
...
class IngestionRouter(BaseRouter):
    ...
    def _setup_routes(self):
        ...
        async def ingest_files_app(
            ...
            if chunking_config:
                chunking_config = json.loads(chunking_config) if chunking_config else None

            if not auth_user.is_superuser:
                for metadata in metadatas or []:
                    if "user_id" in metadata and metadata["user_id"] != str(auth_user.id):
                        raise R2RException(status_code=403, message="Non-superusers cannot set user_id in metadata.")
            ...
            for it, file_data in enumerate(file_datas):
                ...
                document_id = document_ids[it] if document_ids else generate_user_document_id(file_data["filename"], auth_user.id)
                ...
                task = ingest_file.delay(workflow_input)
